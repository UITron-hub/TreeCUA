# openrlhf/agent/exploration/post_processing/summary_overall.py
"""
Generates a concise overall task summary (final_task_summary) from trajectory history.
...
from exploration.prompts import POSTPROCESS_SUMMARY_OVERALL_PROMPT

logger = logging.getLogger(__name__)


class SummaryOverallAgent:
    """Generates a single imperative task instruction summarizing an entire trajectory."""
    ...
    def process(self, data: Dict) -> Optional[Dict]:
        """
        Generate overall task summary for a trajectory.

        Args:
            data: Trajectory manifest dict (must contain 'traj_detail').
        Returns:
            Dict with 'final_task_summary' key, or None on failure.
        """
        ...
        prompt = POSTPROCESS_SUMMARY_OVERALL_PROMPT.format(history=history_str)
